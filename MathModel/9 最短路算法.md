...
在带权图中，从给定起点到终点的所有可行路径中，**边权值之和最小**的一条路径，称为最短路。
## 9.2 求解
### 9.2.1 整数规划

设决策变量为$x_{ij}$，含义为：$\begin{cases} 1,从i点到j点在最短路上 \\ 0,从i点到j点不在最短路上\end{cases}$。
目标函数为$\min \sum_{v_i v_j \in E}w_{ij}x_{ij}$。
...
\sum_{i,i \ne j,j \ne s ,(i, j)\in E }^Nx(i,j)=1 \\ \\
\sum_{i,j \ne i,j \ne t ,(j, i)\in E }^Nx(j,i)=1 
\end{cases}
\end{aligned}
$$
也就是
$$
\begin{cases}  
\displaystyle\sum_{(i,s)\in E}x_{is}=0,\\
\displaystyle\sum_{(t,j)\in E}x_{tj}=0,\\
\displaystyle\sum_{(i,j)\in E}x_{ij}=1,\qquad \forall j\in V\setminus{s},\\
\displaystyle\sum_{(j,i)\in E}x_{ji}=1,\qquad \forall j\in V\setminus{t}.  
\end{cases}  
$$
...
$$
s.t.\begin{cases}
\sum_j x_{ij}-\sum_i x_{ji}=\begin{cases}1,i=s \\
-1, i=t \\ 
0,i\ne s,t
\end{cases}\;, \\
x_{ij}\in\{0,1\}.
